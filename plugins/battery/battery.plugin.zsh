...
function battery_pct_prompt () {
  local pct color watts charging_symbol
  ...
  if battery_is_charging; then
    charging_symbol="${BATTERY_CHARGING:-⚡️}"
    ...
    if [[ -n "$watts" ]]; then
      echo "%{$fg[cyan]%}[${pct}%% – ${watts}${charging_symbol}]%{$reset_color%}"
    else
      echo "%{$fg[cyan]%}[${pct}%% – ${charging_symbol}]%{$reset_color%}"
    fi
  else
    if [[ $pct -gt 50 ]]; then
      color='green'
